...
/* The entry points are named jpeg_prepare/jpeg_decomp (not TJpgDec's stock
 * jd_prepare/jd_decomp) because the ESP32-C5 ROM exports its own TJpgDec
 * under the stock names; a strong linker-script symbol steals them, binding
 * our calls to a ROM decoder whose JDEC layout and pixel format we do not
 * control. Renaming forces this bundled, JD_FORMAT=1 copy to be linked. */
